...
//                 Price = 100f, Quantity = 1, Users_ID = users[1].ID, CategoryId = categories[1].ID,
...
public static class DbSeeder
{
    public static void Clean(ToolLendingContext context)
    {
        ...
        // IMPORTANT: actually delete from DB
        context.SaveChanges();

        // Optional: clear tracked deleted entities
        context.ChangeTracker.Clear();
    }

    public static void Seed(ToolLendingContext context)
    {
        Clean(context); // always start fresh for dev

        // USERS
        var users = new List<User>
        {
            new() { ID = Guid.NewGuid(), Username = "admin", Email = "admin@example.com", Password = "admin123", Role = "Admin" },
            new() { ID = Guid.NewGuid(), Username = "john",  Email = "john@example.com",  Password = "password",  Role = "Customer" }
        };
        context.Users.AddRange(users);
        context.SaveChanges();
